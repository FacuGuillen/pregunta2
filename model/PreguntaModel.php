...
class PreguntaModel
{
            ...
            LEFT JOIN (
                SELECT id_preguntas,
                    CASE 
                        ...
                        ELSE 'normal' 
                    END AS dificultad_real 
                FROM preguntas_usuarios_respuestas 
                GROUP BY id_preguntas
            ) AS dificultad ON dificultad.id_preguntas = p.id_pregunta 
            WHERE c.categoria = ? 
    ...
    public function nuevaCategoriaDisponible($idUsuario)
    {
        ...
        $sql = "SELECT c.categoria 
                FROM categoria c 
                JOIN pregunta p ON c.id_categoria = p.id_categoria 
                WHERE NOT EXISTS (
                    SELECT 1 
                    FROM pregunta_usuarios pu 
                    WHERE pu.id_pregunta = p.id_pregunta AND pu.id_usuario = ?
    ...
    // GESTIÓN DE RESPUESTAS Y USUARIO
